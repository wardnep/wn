@php
    ...
    $win_point = $items->where('result', 'WIN')->sum('tp1') + $items->where('result', 'WIN')->sum('tp2');
    ...
    function drawdown($items)
    {
        ...
        foreach ($items as $item) {
            $equity += $item->result_r1 + $item->result_r2;
    ...
    function expectancy($items)
    {
        ...
        $sum = $items->sum('result_r1') + $items->sum('result_r2');
