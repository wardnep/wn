...
@endphp
@php
    $query = JourneyItem::where('journey_id', $select_journey->id);
    if ($exclude_asia == 'Y') {
        $query->where('journey_id', $select_journey->id)->where('session', '<>', 'Asia');
    }

    if ($exclude_london == 'Y') {
        $query->where('journey_id', $select_journey->id)->where('session', '<>', 'London');
    }

    if ($exclude_london_ny == 'Y') {
        $query->where('journey_id', $select_journey->id)->where('session', '<>', 'London + NY');
    }

    if ($exclude_ny == 'Y') {
        $query->where('journey_id', $select_journey->id)->where('session', '<>', 'NY');
    }
    $items = $query->get();
    ...
    $obd = $weekdays ? number_format($total / $weekdays) : 0;
@endphp
<h4>
<div class="row">
    <div class="col-md-3">
        ...
        Orders/Day <b>{{ $obd }}</b><br />
        <a href="{{ url('journey/chart/'.$select_journey->id.'/'.$exclude_asia) }}" target="_blank">chart</a>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-3">
        ...
        <a href="" class="btn btn-success"><span class="fa fa-file-excel-o" /> Export</a>
    </div>
</div>
</h4>
